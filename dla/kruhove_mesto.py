...
def distance(x,y,kamx,kamy):
    return sqrt((x-kamx)**2+(y-kamy)**2)
...
nusednuti = 0
infloop = 0
while len(houses)!=0 and infloop<SIRKA*200:
    print(infloop)
    infloop +=1
    emigranti = []
    for house in houses:
        x = house.x
        y = house.y
        #distance(x,y,SIRKA//2,VYSKA//2)<(distance(0,SIRKA//2,VYSKA//2,SIRKA//2)*((nusedliku**2-nusednuti**2)/(nusedliku)**2)):
        if house.muzu_sednout() and distance(x,y,SIRKA//2,VYSKA//2)< sqrt((obsah+3)*7/(4*pi)):
            house.sednout()
            len_cesta = house.zarid_cestu()
            emigranti.append(house)
            obsah += len_cesta + len(house.lpozic)
            nusednuti +=1
            infloop = 0
            #print(len(houses))
            #mapa = [[obj.farbe for obj in row] for row in mat]
...
print("usednuti:",nusednuti,"/",nusedliku)
