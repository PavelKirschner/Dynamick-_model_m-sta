...
class Dum(POLE):
    ...
    def soupnem_ho_otaznik(self, max_dis_to_cesta):
        r = dis_to_nearest(self.x,self.y,1)
        if r > max_dis_to_cesta:
            return True
        else:
            return False
        
    def demolice(self):
        for px,py in self.get_lpozic():
            mat[px][py] = Nic(px,py)
...
infloop = 0
while len(houses)!=0 and infloop<SIRKA*5:
    print(infloop)
    print(len(houses))
    infloop +=1
    emigranti = []
...
print("přestěhovaných:", stehovani)
